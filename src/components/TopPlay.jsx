...
import topartists from '../assets/artistPics.json'
  ...
  <div className="w-full flex flex-row items-center hover:bg-[#4c426e] py-2 p-4 rounded-lg cursor-pointer mb-2">
    ...
    <div className="flex-1 flex flex-row justify-between items-center">
      ...
      <div className="flex-1 flex flex-col justify-center mx-3">
        ...
        <Link to={`/artists/${song?.relationships.artists.data[0].id}`}><p className=" text-base text-gray-300 mt-1">{song?.attributes.artistName}</p></Link>
      </div>
    </div>
    <PlayPause
    isPlaying={isPlaying}
      ...
      {topSongs?.map((song, i) => (
        <SwiperSlide key={song?.key} style={{width:'25%' , height:'auto'}} className="shadow-lg rounded-full animate-slideright">
          <Link to={`/artists/${song?.relationships.artists.data[0].id}`}>
            <img src={topartists[i].url} alt={song.attributes.artistName} className="rounded-full w-full object-cover"/>
          </Link>
        </SwiperSlide>
      ))}
      </Swiper>
